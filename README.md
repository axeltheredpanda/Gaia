...
- [x] IA locale Ollama (spec 10) — vérification au démarrage (`localhost:11434`), dégradation propre si absent. Filler vocal TTS (Llama 3.2 3B) pendant les appels cloud Haiku/Sonnet en mode voix ; troisième palier de routing sous Haiku (Llama 3.1 8B) pour reformulation/traduction/contexte conversationnel explicite, heuristique conservatrice documentée dans `router.ts`. Pas de tool calling sur les chemins Ollama.
- [x] Mémoire `style_interaction` (spec 4.8 étendue) — catégorie peripheral pour les préférences d'interaction, injectée intégralement à chaque requête (comme core), extraction Haiku après chaque échange réactivée (`memoryExtraction.ts`), garde-fou core/peripheral préservé sur l'écriture.
